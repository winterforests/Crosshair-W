...
function overlayRenderSize(config) {
...
function overlayBounds() {
  ...
  const size = overlayRenderSize(state.config);
  const px = state.config?.position?.x ?? 0;
  const py = state.config?.position?.y ?? 0;
  const cx = x + width / 2 + px;
  const cy = y + height / 2 + py;
  return {
...
function pushConfigToOverlay() {
  ...
  if (moved) overlayWindow.setBounds(bounds);
  if (sig === lastConfigSig && !moved) return;
  lastConfigSig = sig;
...
function setEnabled(enabled) {
  ...
  state._rebuildTray?.();
  try {
    const cur = readSaveFile() || {};
    writeSaveFile({ ...cur, overlayEnabled: state.enabled, config: state.config, settings: state.settings });
  } catch {}
}
...
app.whenReady().then(() => {
  ...
  if (saved) {
    ...
    if (typeof saved.overlayEnabled === 'boolean') state.enabled = saved.overlayEnabled;
  }
  createMainWindow();
  createTray();
  registerHotkey();
...
ipcMain.handle('overlay:set-config', async (_e, config) => {
  ...
  try {
    const cur = readSaveFile() || {};
    writeSaveFile({ ...cur, config: state.config, overlayEnabled: state.enabled, settings: state.settings });
  } catch {}
  return true;
});
